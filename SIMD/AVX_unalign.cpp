...
int main()
{
	...
	for (int x = 0; x < 1; x++)
	{
		...
		for (int k = 0; k < N; k++)
		{
			...
			for (j = k + 1; j + 7 < N; j += 8)
			{
				__m256 va = _mm256_loadu_ps(&arry[k][j]);
			...
			for (i = k + 1; i < N; i++)
			{
				...
				for (j = k + 1; j + 7 < N; j += 8)
				{
					__m256 vakj = _mm256_loadu_ps(&arry[k][j]);
					__m256 vaij = _mm256_loadu_ps(&arry[i][j]);
		...
		for (int i = N - 2; i >= 0; i--)
		{
			...
			for (j = i + 1; j + 7 < N; j += 8)
			{
				__m256 vaij = _mm256_loadu_ps(&arry[i][j]);//加载该行后面8个值
				__m256 vaan = _mm256_loadu_ps(&anwser[j]);//加载对应的答案值
